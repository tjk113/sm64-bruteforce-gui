...
game = so.get_option_val('game')
filename = so.get_option_val('input_m64')
filename_new = so.get_option_val('output_m64')
start_frame = so.get_option_val('start_frame')
end_frame = so.get_option_val('end_frame')
...
def get_fitness(x, y, z, hspd, coins, fyaw, actn):
    ...
    for option, i in zip(fitness_opt_vals, iter):
        # Skip unused options
        if fitness_opt_vals[option] == None or fitness_opt_vals[option] == '':
            continue
...
for attempt in range(500000):
    ...
    for i in range(500000):
        ...
        if fyaw > 65535:
            ...
            fyaw += 65536
        fitness = get_fitness(x, y, z, hspd, coins, fyaw, actn)
        if fit == 99999:
            fitness = 99990
        if so.get_regularization():
            fitness = fitness + l1ofdiff(m64)*.03 # ?
        if fitness < cur_val:
            last_change = i
            cur_val = fitness
            if fitness < best_val:
